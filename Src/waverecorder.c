...
__IO uint32_t AUDIODataReady = 0, AUDIOBuffOffset = 0, AUDIOOutputReady = 0;
...
uint8_t test_audio_output[WR_BUFFER_SIZE] = {0};
void play_square_wave(void) {
	...
	uint16_t test_audio_output_max[WR_BUFFER_SIZE] = {65535};
	for(int i=0; i < WR_BUFFER_SIZE; i++) {
		test_audio_output_max[i] = 65535;
	}

	uint16_t test_audio_output_zero[WR_BUFFER_SIZE] = {0};
	for(int i=0; i < WR_BUFFER_SIZE; i++) {
			test_audio_output_zero[i] = 3267;
	}

	if(BSP_AUDIO_OUT_Play((uint16_t*)&test_audio_output[0], WR_BUFFER_SIZE) != AUDIO_OK){
	...
	{
		//switch wave amplitude every 1ms. So f = 2000Hz
		if(TimeRecBase > 1){
			if(a==1)
				a=2;
			else
		...
		{
			if(a==1)
				memcpy((uint16_t*)&test_audio_output[0],test_audio_output_zero,WR_BUFFER_SIZE);
			if(a==2)
				memcpy((uint16_t*)&test_audio_output[0],test_audio_output_max,WR_BUFFER_SIZE);
		}

		if(buffer_offset == BUFFER_OFFSET_FULL)
		{
			if(a==1)
				memcpy((uint16_t*)&test_audio_output[WR_BUFFER_SIZE/2],test_audio_output_zero,WR_BUFFER_SIZE);

			if(a==2)
				memcpy((uint16_t*)&test_audio_output[WR_BUFFER_SIZE/2],test_audio_output_max,WR_BUFFER_SIZE);
		}
	}
